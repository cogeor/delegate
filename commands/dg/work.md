...
5. **One loop = plan -> implement -> test -> YOU commit -> doc update (non-blocking)** — never skip the commit.
6. **One loop = one commit scope** — If the planner produces LOOPS.yaml with multiple entries, each entry is a separate loop with its own commit. Never combine multiple LOOPS.yaml entries into a single commit. Never skip entries.
...
3. Spawn **dg-planner** agent to produce a plan
4. Check planner output:
   - **If `LOOPS.yaml` exists** in the plan folder:
     1. Read LOOPS.yaml entries (each entry has: id, slug, draft summary, dependencies)
     2. Show the loop manifest to the user (count, names, dependency order)
     3. For each entry in dependency order, run **Execute Loop** (Step 3):
        - Create a DRAFT.md for the entry from its summary in LOOPS.yaml
        - Each loop gets its own folder, plan, implementation, test, and commit
     4. Report aggregate results when all loops complete
   - **If only `PLAN.md` exists** (no LOOPS.yaml):
     1. Show plan to user, then proceed to **Execute Loop** (Step 3) as a single loop

### Mode: draft

1. Find draft file (check arg path, then `./plan_draft.md`, then `.delegate/plan_draft.md`)
2. Create plan folder: `.delegate/loop_plans/{YYYYMMDD-HHMMSS}-{slug}/`
3. Copy draft into plan folder, spawn **dg-planner** agent
4. Check planner output:
   - **If `LOOPS.yaml` exists** in the plan folder:
     1. Read LOOPS.yaml entries (each entry has: id, slug, draft summary, dependencies)
     2. Show the loop manifest to the user (count, names, dependency order)
     ...
     4. Report aggregate results when all loops complete
   - **If only `PLAN.md` exists** (no LOOPS.yaml):
     1. Proceed to **Execute Loop** (Step 3) with that draft as a single loop

### Mode: loops / range / all

1. Find active plan: `ls -td .delegate/loop_plans/*/ | head -1` (or use `--plan` path)
2. Load LOOPS.yaml manifest
   (LOOPS.yaml may come from study-generated drafts in `loop_plans/` or from planner decomposition during prompt/draft modes)
...
When the planner decomposes work into multiple loops, it produces a `LOOPS.yaml` file:
...
loops:
  - id: "01"
    ...
    summary: |
      What this loop accomplishes (used as DRAFT.md content)
    depends_on: []           # list of loop IDs that must complete first
...
The orchestrator uses this to determine execution order (respecting `depends_on`) and tracks status as each loop completes.
...
1. Spawn **dg-doc-generator** agent:
   - Input: commit hash from Phase 4
   - The agent runs `git diff HEAD~1 HEAD --name-only` to identify changed source files
   - For each changed source file, generates or updates documentation in `.doc/{path}.md`
2. This phase is **non-blocking**: if documentation generation fails, log the failure but do NOT fail the loop or roll back the commit
3. Update STATUS.md: `phase: complete` (remains complete regardless of doc generation outcome)
...
━━━━━━━━━━━
  ...
  ? Documenting  → {N} files updated (or "skipped" on failure)
```
